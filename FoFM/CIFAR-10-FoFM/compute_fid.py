...
alpha = FLAGS.alpha if FLAGS.alpha > 0 else checkpoint.get("alpha", 1.0)
N_steps = checkpoint.get("N_steps", 100)
print(f"Using alpha={alpha}, N_steps={N_steps}")
try:
...
# Define the integration method if euler is used (only for standard flow matching)
if alpha == 1.0 and FLAGS.integration_method == "euler":
...
def gen_1_img(unused_latent):
    with torch.no_grad():
        ...
        if alpha == 1.0:
            ...
            traj = sample_caputo(new_net, x, alpha=alpha, N=N_steps, device=device)
    # .view([-1, 3, 32, 32]).clip(-1, 1)
