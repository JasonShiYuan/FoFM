...
# ====================== 3. 网络：条件轻量 UNet（~4.95M，全部通道为 8 的倍数）======================
...
class ConditionalLightUNet(nn.Module):
    def __init__(self, t_dim=128, num_classes=10):
        ...
        # 所有通道数均调整为 8 的倍数，满足 GroupNorm(8, ...) 要求
        ...
        self.bottleneck3 = ResBlock(176, 176)        # 新增
...
def train_mnist_feature_extractor(device='cuda', feat_dim=128, epochs=16,
    ...
    if os.path.exists(save_path):
        print(f"[FID] Loading pre-trained feature extractor: {save_path}")
...
def sample_caputo(model, x0, alpha, N=100, device='cuda', y=None):
    ...
    if y is None:
        raise ValueError("条件生成必须提供 y (0-9)")
    if isinstance(y, int):
